...
resource "aws_instance" "control_plane" {
  ...
  tags = {
    Name = "${var.region}-${var.env_name}-oik8s-control-plane-${count.index}"
  }
}
...
resource "aws_network_interface" "control_plane_secondary_nic" {
  ...
  tags = {
    Name = "${var.region}-${var.env_name}-oik8s-control-plane-secondary-nic-${count.index}"
  }
}
...
resource "aws_instance" "worker_node" {
  ...
  tags = {
    Name = "${var.region}-${var.env_name}-oik8s-worker-node-${count.index}"
  }
}
...
resource "aws_network_interface" "worker_node_secondary_nic" {
  ...
  tags = {
    Name = "${var.region}-${var.env_name}-oik8s-worker-node-secondary-nic-${count.index}"
  }
}
...
resource "aws_instance" "gpu_node" {
  ...
  tags = {
    Name = "${var.region}-${var.env_name}-oik8s-gpu-node-${count.index}"
  }
}
...
resource "aws_network_interface" "gpu_node_secondary_nic" {
  ...
  tags = {
    Name = "${var.region}-${var.env_name}-oik8s-gpu-node-secondary-nic-${count.index}"
  }
}
...
resource "aws_instance" "bastion_host" {
  ...
  user_data = <<-EOF
            ...
            hostnamectl set-hostname "${var.env_name}-oik8s-bastion"
            echo "127.0.0.1 ${var.env_name}-oik8s-bastion" >> /etc/hosts
            apt update
  ...
  tags = {
    Name = "${var.region}-${var.env_name}-oik8s-bastion"
  }
}  network_interface_id = aws_network_interface.control_plane_secondary_nic[count.index].id
  device_index         = 1 # 0 is always the primary/default interface
}
